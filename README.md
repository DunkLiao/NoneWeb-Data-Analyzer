...
- 🌗 **全域深淺色主題切換 (Dark / Light Mode)**：
  - 支援一鍵無縫切換現代深色極客模式與專業高對比淺色模式，偏好設定自動記憶於本機。
  - ECharts 10+ 款視覺化圖表全自動自適應主題配色、座標格線與提示框對比。
- 📊 **多維視覺化統計探索 (10+ 種專業 EDA 圖形)**：
  - **數值型**：動態直方圖 + 高斯核密度曲線 (KDE, 支援 Bin 分箱滑桿)、五數摘要箱形圖 (Boxplot, IQR 異常值偵測)、累積分布函數 (CDF)、**常態機率 Q-Q 分位圖 (Normal Q-Q Plot, 檢驗高斯常態分佈與 R² 擬合判定)**。
  - **類別型**：頻率分布長條圖、占比環形圖 (Donut)、**二八法則柏拉圖 (Pareto Chart, 結合累計百分比與 80% 核心門檻線)**、**矩形樹狀面積圖 (Treemap, 解決高維度多類別視覺擁擠)**。
  - **多維與變量相關性**：全特徵皮爾森相關矩陣熱力圖 (Pearson Correlation)、雙變量互動散佈圖 (Scatter Plot + 迴歸趨勢線)、**四維多變量氣泡圖 (4D Bubble Chart, 融合 X/Y/氣泡半徑/類別分組色彩)**、**多特徵統計指紋雷達圖 (Feature Profile Radar Chart, 歸一化對比多欄位分佈特徵)**。
