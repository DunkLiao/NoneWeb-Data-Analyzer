...
export const ColumnSelector: React.FC<ColumnSelectorProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex flex-col h-full shadow-sm">
      <div className="text-xs font-semibold text-slate-300 mb-2 px-1">欄位清單 ({columns.length})</div>
      ...
      <div className="flex-1 overflow-y-auto space-y-1 pr-1 max-h-[520px]">
        ...
        {filteredColumns.length === 0 && (
          <div className="text-center py-6 text-xs text-slate-500">無符合條件之欄位</div>
        )}
      </div>
    </div>
  );
