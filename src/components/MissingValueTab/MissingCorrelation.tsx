  ...
  if (columns.length < 2) {
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center">
        <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-slate-800/80 text-emerald-400 mb-3">
          <CheckCircle2 className="w-6 h-6" />
        </div>
        <h4 className="text-sm font-semibold text-slate-200">無須進行缺值共現分析</h4>
  ...
  const option: EChartsOption = {
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        return `
          <div style="font-size: 13px; font-weight:600; color:#f8fafc;">${col1} ↔ ${col2}</div>
          <div style="font-size: 12px; color:#cbd5e1; margin-top:3px;">缺值共現係數: <strong style="color: ${val > 0.3 ? '#ef4444' : val < -0.3 ? '#3b82f6' : '#cbd5e1'}">${val}</strong></div>
          <div style="font-size: 11px; color:#94a3b8; margin-top:2px;">(接近 +1 表示通常一起缺失，接近 -1 表示交替缺失)</div>
        `;
      },
    ...
    visualMap: {
      ...
      bottom: '0%',
      inRange: {
        color: ['#3b82f6', '#1e293b', '#ef4444'], // Blue (-1) to Dark (0) to Red (+1)
      },
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            衡量兩欄位是否「同時發生缺失」。係數接近 <strong className="text-red-400">+1</strong> 代表常常一同遺失；
            接近 <strong className="text-blue-400">-1</strong> 代表一欄缺值時另一欄通常完整。
          </p>
        </div>
      </div>
